...
class TppConnectorTest {
    ...
    void requestCertificate() throws VCertException {
        ...
        TppConnector.ReadZoneConfigurationRequest expectedRZCRequest = new TppConnector.ReadZoneConfigurationRequest("\\VED\\Policy\\\\VED\\Policy\\myZone");
        when(tpp.readZoneConfiguration(eq(expectedRZCRequest), eq(API_KEY)))
                .thenReturn(new TppConnector.ReadZoneConfigurationResponse()
                    .policy(new ServerPolicy()
                            .subject(new ServerPolicy.Subject()
                                    .organizationalUnit(new LockableValues<String>(false, Collections.singletonList("OU")))
        ...
        CertificateRequest request = new CertificateRequest()
                .subject(new CertificateRequest.PKIXName()
                        ...
                        .province(Collections.singletonList("Nevada"))
        ...
        String requestId = classUnderTest.requestCertificate(request, classUnderTest.getPolicyDN(zoneTag));
